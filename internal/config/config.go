...
type Config struct {
	...
	Path              string            `toml:"-"`
	TodoistApiToken   string            `toml:"api_token"`
	TodoistBearerAuth string            `toml:"bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("TODOIST_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "todoist-aum", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("TODOIST_API_TOKEN"); v != "" {
		cfg.TodoistApiToken = v
		cfg.AuthSource = "env:TODOIST_API_TOKEN"
	}
	if v := os.Getenv("TODOIST_BEARER_AUTH"); v != "" {
		cfg.TodoistBearerAuth = v
		cfg.AuthSource = "env:TODOIST_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.TodoistApiToken != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.TodoistBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("TODOIST_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.TodoistApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:TODOIST_API_TOKEN"
		}
		return "Bearer " + c.TodoistApiToken
	}
	if c.TodoistBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:TODOIST_BEARER_AUTH"
		}
		return "Bearer " + c.TodoistBearerAuth
	}
	if c.AccessToken != "" {
