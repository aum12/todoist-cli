...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  todoist-aum doctor
  todoist-aum doctor --json
  todoist-aum doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export TODOIST_API_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "TODOIST_API_TOKEN")
				if os.Getenv("TODOIST_API_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "TODOIST_API_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("TODOIST_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "TODOIST_API_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "TODOIST_API_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "TODOIST_BEARER_AUTH")
				if os.Getenv("TODOIST_BEARER_AUTH") != "" {
					authEnvSet = append(authEnvSet, "TODOIST_BEARER_AUTH")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("TODOIST_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "TODOIST_BEARER_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "TODOIST_BEARER_AUTH optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "todoist-aum"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'todoist-aum sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'todoist-aum sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'todoist-aum sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'todoist-aum sync' to refresh."
	}
	return report
}
