...
Resource scoping:
  ...
  todoist-aum sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  todoist-aum sync --full

  # Incremental sync: only records from the last 7 days
  todoist-aum sync --since 7d
  ...
  todoist-aum sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("todoist-aum")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/todoist-aum/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "folders":
		return db.UpsertFolders(data)
	case "labels":
		return db.UpsertLabels(data)
	case "location-reminders":
		return db.UpsertLocationReminders(data)
	case "payments":
		return db.UpsertPayments(data)
	case "projects_archive":
		return db.UpsertProjectsArchive(data)
	case "collaborators":
		return db.UpsertCollaborators(data)
	case "join":
		return db.UpsertJoin(data)
	case "projects_unarchive":
		return db.UpsertProjectsUnarchive(data)
	case "reminders":
		return db.UpsertReminders(data)
	case "sections":
		return db.UpsertSections(data)
	case "sections_archive":
		return db.UpsertSectionsArchive(data)
	case "sections_unarchive":
		return db.UpsertSectionsUnarchive(data)
	case "tasks":
		return db.UpsertTasks(data)
	case "close":
		return db.UpsertClose(data)
	case "move":
	...
	case "uploads":
		return db.UpsertUploads(data)
	case "workspaces":
		return db.UpsertWorkspaces(data)
	case "workspaces_projects":
		return db.UpsertWorkspacesProjects(data)
	case "users":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"projects-archived":  "/api/v1/projects/archived",
		"reminders":          "/api/v1/reminders",
		"sections":           "/api/v1/sections",
		"tasks":              "/api/v1/tasks",
		"workspaces":         "/api/v1/workspaces",
		"workspaces-users":   "/api/v1/workspaces/users",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		...
		{Name: "workspaces_projects", ParentTable: "workspaces", ParentIDParam: "workspace_id", PathTemplate: "/api/v1/workspaces/{workspace_id}/projects/active", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "workspace_id", Field: "id"},
		}},
		{Name: "workspaces_projects", ParentTable: "workspaces", ParentIDParam: "workspace_id", PathTemplate: "/api/v1/workspaces/{workspace_id}/projects/archived", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "workspace_id", Field: "id"},
		}},
	}
}
