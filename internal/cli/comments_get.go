...
func newCommentsGetCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "get",
		Aliases:     []string{"list"},
		Short:       "Get all comments for a given task or project. Exactly one of `task_id` or `project_id` arguments is required.",
		Example:     "  todoist-aum comments get",
		Annotations: map[string]string{"pp:endpoint": "comments.get", "pp:method": "GET", "pp:path": "/api/v1/comments", "mcp:read-only": "true"},
