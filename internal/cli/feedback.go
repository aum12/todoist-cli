...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "todoist-aum")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TODOIST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TODOIST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "todoist-aum/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/todoist-aum/feedback.jsonl.
When ` + "`TODOIST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TODOIST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  todoist-aum feedback list
  todoist-aum feedback list --limit 5
  todoist-aum feedback list --json`,
