...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  todoist-aum workflow archive

  # Full re-archive (ignore previous sync state)
  todoist-aum workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"activities", "backups", "comments", "labels", "labels-shared", "location-reminders", "projects", "projects-archived", "reminders", "sections", "tasks", "workspaces", "workspaces-users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  todoist-aum workflow status

  # Show status as JSON
  todoist-aum workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("todoist-aum")
			}
