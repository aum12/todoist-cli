...
In local mode: searches locally synced data only.`,
  ...
  todoist-aum search "error timeout"

  # Force local search only
  todoist-aum search "payment failed" --data-source local

  # Search a specific resource type locally
  todoist-aum search "critical" --type transactions --data-source local

  # JSON output for piping
  todoist-aum search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v1/labels/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/v1/labels/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'todoist-aum sync' first to populate the local database.", err)
			}
			defer db.Close()
