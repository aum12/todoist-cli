...
Highlights (not in the official API docs):
  ...
  • review   Retrospective rollup with prior-period delta. '--window day' covers end-of-day check-in; '--window week' covers Sunday review; '--window month' covers month rollup.
  • agenda   Today's tasks plus the overdue tail plus a windowed lookahead with priority and project context — composed from the local store in one pass.
  • triage   For each Inbox item, propose the most likely (project, label, section) by matching historical co-occurrence in the local store, then apply the reviewed plan atomically.
  • reschedule-cascade   Postpone every task matching a Todoist filter by a relative shift, previewing per-task new due strings, recurring-next-instance shifts, and deadline-violation warnings before any write.
  • filter-batch   Bulk complete, move, or relabel every task matching a Todoist filter, with a before/after diff plan that requires explicit apply.
  • focus   'focus set --top N --reason <why>' picks the highest-priority/deadline-tightest tasks and labels them @focus-today; 'focus show' returns just those; 'focus clear' removes the label at end-of-day.
  • near   Takes a label or context name (walmart, home, office), returns open tasks tagged with that label OR in any matching project, ranked by priority and age — agent-shaped envelope for geofence-triggered …
  • productivity-trend   Rollup over local completed_tasks with group-by dimensions (label, hour-of-day) the Todoist stats endpoint does not expose.
  • workload   Cross-project rollup of open tasks per collaborator inside a workspace, by priority, overdue-age bucket, and project.
  • reschedule-history   Walk cached activity events for a task (or filter) and show every time the due date moved, by how much, on which day.
  • stale-review   Surface open tasks that are old and inactive; annotate each with a mechanical suggested action (close-as-obsolete / move-to-someday / break-down / reschedule:+7d / manual-review) from heuristics over…

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'todoist-aum doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("todoist-aum {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'todoist-aum profile list')")
	...
	// folders + workspaces removed — token has no workspace memberships
	rootCmd.AddCommand(newLabelsCmd(flags))
	rootCmd.AddCommand(newLocationRemindersCmd(flags))
	...
	rootCmd.AddCommand(newUploadsCmd(flags))
	...
	rootCmd.AddCommand(newNovelAgendaCmd(flags))
	rootCmd.AddCommand(newNovelCaptureCmd(flags))
	rootCmd.AddCommand(newNovelFilterBatchCmd(flags))
	rootCmd.AddCommand(newNovelFocusCmd(flags))
	rootCmd.AddCommand(newNovelNearCmd(flags))
	rootCmd.AddCommand(newNovelProductivityTrendCmd(flags))
	rootCmd.AddCommand(newNovelRescheduleCascadeCmd(flags))
	rootCmd.AddCommand(newNovelRescheduleHistoryCmd(flags))
	rootCmd.AddCommand(newNovelReviewCmd(flags))
	rootCmd.AddCommand(newNovelStaleReviewCmd(flags))
	rootCmd.AddCommand(newNovelTriageCmd(flags))
	rootCmd.AddCommand(newNovelWorkloadCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newActivitiesPromotedCmd(flags))
	rootCmd.AddCommand(newIdMappingsPromotedCmd(flags))
	rootCmd.AddCommand(newNotificationSettingPromotedCmd(flags))
	rootCmd.AddCommand(newRevokePromotedCmd(flags))
