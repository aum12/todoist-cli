...
// Package cliutil contains shared helpers used across the CLI. Helpers live
// in their own package (not in package cli) to avoid symbol collisions with
// commands in package cli. Callers import as `cliutil` and invoke
// `cliutil.FanoutRun(...)`, `cliutil.CleanText(...)`, etc.
package cliutil
